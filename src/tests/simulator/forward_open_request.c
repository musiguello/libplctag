...
static int unmarshal_and_validate_forward_open_request(context_s *context, slice_s raw_request, forward_open_request_s *fo_req);
static slice_s marshal_forward_open_response(context_s *context, slice_s output_buf, forward_open_request_s *fo_req);
...
slice_s handle_forward_open_request(context_s *context, slice_s raw_request)
{
    ...
    rc = unmarshal_and_validate_forward_open_request(context, raw_request, &fo_req);
    ...
    return marshal_forward_open_response(context, context->buffer, &fo_req);
}


#define BUMP(n) do { offset += (n); } while(0);

int unmarshal_and_validate_forward_open_request(context_s *context, slice_s raw_request, forward_open_request_s *fo_req)
{
    int offset = 0;

    info("unmarshal_and_validate_forward_open_request(): decoding and validating packet:");
    slice_dump(raw_request);

    /* check the size.  This is tricky because there are variable length fields. */
    if(raw_request.len < FORWARD_OPEN_REQ_SIZE_BASE) {
        info("unmarshal_and_validate_forward_open_request(): Payload is insufficient for forward open request data!");
    ...
    if(fo_req->cm_service_code == CIP_CMD_FORWARD_OPEN) {
        ...
        context->max_request_size = fo_req->orig_to_targ_conn_params & 0x1FF;

        if(context->max_request_size > CIP_MAX_REQ) {
            info("unmarshal_and_validate_forward_open_request(): Illegal request buffer size requested: %d!", context->max_request_size);
            return PLCTAG_ERR_TOO_LARGE;
        }
    } else {
        fo_req->orig_to_targ_conn_params = get_uint32_le(raw_request, offset); BUMP(4);
        context->max_request_size = fo_req->orig_to_targ_conn_params & 0xFFFF;

        if(context->max_request_size > CIP_MAX_REQ_EX) {
            info("unmarshal_and_validate_forward_open_request(): Illegal extended request buffer size requested: %d!", context->max_request_size);
            return PLCTAG_ERR_TOO_LARGE;
        }
    }
    ...
    if(fo_req->cm_service_code == CIP_CMD_FORWARD_OPEN) {
        ...
        context->max_response_size = fo_req->targ_to_orig_conn_params & 0x1FF;

        if(context->max_response_size > CIP_MAX_REQ) {
            info("unmarshal_and_validate_forward_open_request(): Illegal response buffer size requested: %d!", context->max_response_size);
            return PLCTAG_ERR_TOO_LARGE;
        }
    } else {
        ...
        if(context->max_response_size > CIP_MAX_REQ_EX) {
            info("unmarshal_and_validate_forward_open_request(): Illegal extended response buffer size requested: %d!", context->max_response_size);
            return PLCTAG_ERR_TOO_LARGE;
        }
    }
    ...
    if(offset > raw_request.len) {
        info("unmarshal_and_validate_forward_open_request(): Payload after parsing is insufficient for forward open request data!");
        return PLCTAG_ERR_TOO_SMALL;
    }

    if(fo_req->cm_req_path_size != 2) {
        info("unmarshal_and_validate_forward_open_request(): path length to the CM is not two!");
    ...
    if(slice_at(fo_req->cm_req_path,0) != 0x20 || slice_at(fo_req->cm_req_path,1) != 0x06 || slice_at(fo_req->cm_req_path,2) != 0x24 || slice_at(fo_req->cm_req_path,3) != 0x01) {
        info("unmarshal_and_validate_forward_open_request(): path to the CM is not valid!");
        return PLCTAG_ERR_BAD_PARAM;
    }

    if(fo_req->secs_per_tick != (uint8_t)FO_PARAM_SECS_PER_TICK) {
        info("unmarshal_and_validate_forward_open_request(): secs per tick param should be %d but was %d!", FO_PARAM_SECS_PER_TICK, fo_req->secs_per_tick);
        return PLCTAG_ERR_BAD_PARAM;
    }

    if(fo_req->timeout_ticks != (uint8_t)FO_PARAM_TIMEOUT_TICKS) {
        info("unmarshal_and_validate_forward_open_request(): timeout ticks param should be %d but was %d!", FO_PARAM_TIMEOUT_TICKS, fo_req->timeout_ticks);
        return PLCTAG_ERR_BAD_PARAM;
    }

    if(fo_req->orig_to_targ_conn_id != 0) {
        info("unmarshal_and_validate_forward_open_request(): originator to target connection ID param should be zero but was %d!", fo_req->orig_to_targ_conn_id);
        return PLCTAG_ERR_BAD_PARAM;
    }

    /* this comes from the client so anything other than zero is good. */
    if(fo_req->targ_to_orig_conn_id == 0) {
        info("unmarshal_and_validate_forward_open_request(): originator to target connection ID param should not be zero but was!");
        return PLCTAG_ERR_BAD_PARAM;
    }
    ...
    /* save the connection serial number. */
    context->client_connection_seq = fo_req->conn_serial_number;

    info("unmarshal_and_validate_forward_open_request(): conn_serial_number=%d", fo_req->conn_serial_number);

//    if(fo_req->conn_serial_number != FO_PARAM_CONN_SERIAL_NUMBER) {
//        info("unmarshal_and_validate_forward_open_request(): connection serial number param should be %d but was %d!", FO_PARAM_CONN_SERIAL_NUMBER, fo_req->conn_serial_number);
//        return PLCTAG_ERR_BAD_PARAM;
//    }

    if(fo_req->orig_vendor_id != FO_PARAM_VENDOR_ID) {
        info("unmarshal_and_validate_forward_open_request(): vendor ID param should be %d but was %d!", FO_PARAM_VENDOR_ID, fo_req->orig_vendor_id);
        return PLCTAG_ERR_BAD_PARAM;
    }

    if(fo_req->orig_serial_number != FO_PARAM_ORIG_SERIAL_NUMBER) {
        info("unmarshal_and_validate_forward_open_request(): originator serial number param should be %d but was %d!", FO_PARAM_ORIG_SERIAL_NUMBER, fo_req->orig_serial_number);
        return PLCTAG_ERR_BAD_PARAM;
    }

    if(fo_req->conn_timeout_multiplier != FO_PARAM_CONN_TIMEOUT_MULTIPLIER) {
        info("unmarshal_and_validate_forward_open_request(): connection timeout multiplier param should be %d but was %d!", FO_PARAM_CONN_TIMEOUT_MULTIPLIER, fo_req->conn_timeout_multiplier);
        return PLCTAG_ERR_BAD_PARAM;
    }

    if(fo_req->reserved[0] != 0) {
        info("unmarshal_and_validate_forward_open_request(): reserved byte 0 should be zero but was %d!", fo_req->reserved[0]);
        return PLCTAG_ERR_BAD_PARAM;
    }

    if(fo_req->reserved[1] != 0) {
        info("unmarshal_and_validate_forward_open_request(): reserved byte 1 should be zero but was %d!", fo_req->reserved[0]);
        return PLCTAG_ERR_BAD_PARAM;
    }

    if(fo_req->reserved[2] != 0) {
        info("unmarshal_and_validate_forward_open_request(): reserved byte 2 should be zero but was %d!", fo_req->reserved[0]);
        return PLCTAG_ERR_BAD_PARAM;
    }

    if(fo_req->orig_to_targ_rpi != FO_PARAM_RPI) {
        info("unmarshal_and_validate_forward_open_request(): originator to target RPI param should be %d but was %d!", FO_PARAM_RPI, fo_req->orig_to_targ_rpi);
        return PLCTAG_ERR_BAD_PARAM;
    }

    if(fo_req->cm_service_code == CIP_CMD_FORWARD_OPEN) {
        if(fo_req->orig_to_targ_conn_params != (uint32_t)FO_PARAM_CONN_PARAMS) {
            info("unmarshal_and_validate_forward_open_request(): originator to target connection param should be %d but was %d!", FO_PARAM_CONN_PARAMS, fo_req->orig_to_targ_conn_params);
            return PLCTAG_ERR_BAD_PARAM;
        }
    } else {
        if(fo_req->orig_to_targ_conn_params != FO_PARAM_CONN_PARAMS_EX) {
            info("unmarshal_and_validate_forward_open_request(): originator to target connection param should be %d but was %d!", FO_PARAM_CONN_PARAMS_EX, fo_req->orig_to_targ_conn_params);
            return PLCTAG_ERR_BAD_PARAM;
        }
    }

    if(fo_req->targ_to_orig_rpi != FO_PARAM_RPI) {
        info("unmarshal_and_validate_forward_open_request(): target to originator RPI param should be %d but was %d!", FO_PARAM_RPI, fo_req->targ_to_orig_rpi);
        return PLCTAG_ERR_BAD_PARAM;
    }

    if(fo_req->cm_service_code == CIP_CMD_FORWARD_OPEN) {
        if(fo_req->targ_to_orig_conn_params != (uint32_t)FO_PARAM_CONN_PARAMS) {
            info("unmarshal_and_validate_forward_open_request(): target to originator connection param should be %d but was %d!", FO_PARAM_CONN_PARAMS, fo_req->targ_to_orig_conn_params);
            return PLCTAG_ERR_BAD_PARAM;
        }
    } else {
        if(fo_req->targ_to_orig_conn_params != FO_PARAM_CONN_PARAMS_EX) {
            info("unmarshal_and_validate_forward_open_request(): target to originator connection param should be %d but was %d!", FO_PARAM_CONN_PARAMS_EX, fo_req->targ_to_orig_conn_params);
            return PLCTAG_ERR_BAD_PARAM;
        }
    }

    if(fo_req->transport_class != (uint8_t)CIP_TRANSPORT_EXPLICIT) {
        info("unmarshal_and_validate_forward_open_request(): transport class param should be %d but was %d!", CIP_TRANSPORT_EXPLICIT, fo_req->transport_class);
        return PLCTAG_ERR_BAD_PARAM;
    }
    ...
    if(fo_req->path_size * 2 != (uint8_t)MESSAGE_ROUTER_PATH_SIZE) {
        info("unmarshal_and_validate_forward_open_request(): message router path param should be %d bytes long but was %d!", MESSAGE_ROUTER_PATH_SIZE, fo_req->transport_class);
        return PLCTAG_ERR_BAD_PARAM;
    }
    ...
    for(int i=0; i < MESSAGE_ROUTER_PATH_SIZE; i++) {
        if(slice_at(fo_req->path, i) != message_router_path[i]) {
            info("unmarshal_and_validate_forward_open_request(): message router path byte %d should be %d but was %d!", i, message_router_path[i] , slice_at(fo_req->path, i));
            return PLCTAG_ERR_BAD_PARAM;
        }
    }
...
slice_s marshal_forward_open_response(context_s *context, slice_s output_buf, forward_open_request_s *fo_req)
{
    uc_cpf_s cpf_resp;
    slice_s tmp_out;
    slice_s cpf_header;
    ...
    if(slice_err(cpf_header) != PLCTAG_STATUS_OK) {
        info("marshal_forward_open_response(): Unable to marshal CPF header, error %s!", plc_tag_decode_error(slice_err(cpf_header)));
        return cpf_header;
    }
    ...
    if(tmp_out.len < FORWARD_OPEN_RESP_SIZE) {
        info("marshal_forward_open_response() output buffer has insufficient space for forward open response fields!");
